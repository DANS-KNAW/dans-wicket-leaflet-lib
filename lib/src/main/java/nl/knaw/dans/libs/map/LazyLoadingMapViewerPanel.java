...
/**
 * When the production/calculation of the markers takes to much time and the page loading takes to long (>2sec.) You want to have the page loaded and shown
 * before the time consuming work is started. After the work is done the page is updated via AJAX
 */
public abstract class LazyLoadingMapViewerPanel extends MapViewerPanel {
    private static final Logger logger = LoggerFactory.getLogger(LazyLoadingMapViewerPanel.class);
     ...
     * @param id
     */
    public LazyLoadingMapViewerPanel(String id) {
        super(id);

        /**
         * Note that respond is called after OnDomReady, see renderHead the AjaxLazyLoadPanel is using the same approach
         */
    ...
    /**
     * The indicator must be created here instead of in the constructor otherwise page reload is not handled correctly
     */
    @Override
    protected void onBeforeRender() {
        Component newIndicator = new Label("indicator", "<span>Loading...<img alt=\"Loading...\" src=\""
                + RequestCycle.get().urlFor(AbstractDefaultAjaxBehavior.INDICATOR) + "\"/></span>").setEscapeModelStrings(false).setOutputMarkupId(true);
        if (indicator != null) {
    ...
    protected String getInitializationJS() {
        ...
        script += JS_MAPVIEWER_OBJ + ".init(\'" + id + "\', {clustering:true});\n";

        return script;
    }

    /**
     * Overide this one with your own code This would be the time consuming action for which we need the lazy loading
     * 
     * @return
     */
